...
1. **AVB 校验失败** — LK 的 AVB 独立校验 gz 分区，读取无效 LBA 失败 → 重启
2. **LK/kernel 阶段 UFS 崩溃** — preloader 正常返回错误，但后续阶段读取无效 LBA 时控制器崩溃
...
如果仍然重启，可能需要修改 LK/kernel 的 UFS 驱动（部分 preloader 不校验 LK，可直接修改）。
